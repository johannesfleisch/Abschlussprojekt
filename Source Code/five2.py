...
Beschreibung: Kurzes Programm zum Plotten des saisonalen und räumlichen Mittels des Niederschlages über 1979-2019.
"""
...
def plot_violinplot_rain():
    """
    **Titel: Funktion plotten** 

    *Beschreibung: Plotten des saisonalen und räumlichen Mittels der Niederschlagsrate über 1979-2019*

    :variable win_rain: wandelt Winter-Daten für Niederschlagsraten um

    :variable spr_rain: wandelt Frühlings-Daten für Niederschlagsraten um

    :variable sum_rain: wandelt Sommer-Daten für Niederschlagsraten um

    :variable fal_rain: wandelt Herbst-Daten für Niederschlagsraten um

    :variable rain: DataFrame

    :variable axrain: bespielt Diagramm mit Daten

    :method axrain.set_title: erstellt Titel

    :method axrain.grid: erstellt Raster

    :method figrain.savefig: speichert den Plot 
    """
    # convert 3D-Array into 1D-Array
    win_rain = WINTER_R.ravel()
    spr_rain = SPRING_R.ravel()
    sum_rain = SUMMER_R.ravel()
    fal_rain = FALL_R.ravel()

    rain = pd.DataFrame(
        {
            "Jahreszeiten": ["Winter", "Frühling", "Sommer", "Herbst"],
            "Niederschlagsrate in [g/dm²/s]": [
                win_rain,
                spr_rain,
                sum_rain,
                fal_rain,
    ...
    rain = rain.explode("Niederschlagsrate in [g/dm²/s]")
    rain["Niederschlagsrate in [g/dm²/s]"] = rain["Niederschlagsrate in [g/dm²/s]"].astype("float")
    axrain = sns.violinplot(x="Jahreszeiten", y="Niederschlagsrate in [g/dm²/s]", data=rain, gridsize=100)
    axrain.set_title("Saisonle Mittel der Niederschlagsraten über 1979-2019 als Percentile")
    axrain.grid()

    figrain = axrain.get_figure()
    figrain.savefig("E2_Niederschlagsrate_Percentile_1979-2019.png")
